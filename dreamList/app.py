...
def register():
    if request.method == 'POST':
        ...
        if search != None:
            flash('Bu email ile eşleşen bir hesap zaten var!')
...
def create():
    if 'email' in session:
        ...
        if request.method == 'POST':
            ...
            new_dream = Dreams(dream = dream, detail = detail, quote = quote, dreamer_id = me.id)
...
@app.route('/detail')
def detail():
    return '<h1>Merhaba Burası Detay Sayfası</h1>'
